...
- **현재 BUILD 마커**: `gui.py` 안 `BUILD = '2026-07-08.3 (rectify-fix+per-tab-folders+hparam-grid-expand)'`
- **가장 최근 커밋 계열**: rectify 무음 예외 버그 수정 + 탭별 데이터 폴더 지정(전처리/평가/후처리) +
  하이퍼파라미터 탐색 그리드 확장(lambda 3단계 + grad_no_blur) + stage1 top_k 기본값 5;
  그 이전: 설정 자동저장 + 절대경로 수정 + 체크포인트별 하이퍼파라미터 로그/복원
...
**핵심 배선 규칙**:
- `CONFIG_KEYS`(gui.py 상단, **53개**) = 모든 탭 위젯의 순서. `ordered_inputs`로 콜백에 전달.
...
python tests/test_hparam_search.py        # canonicalize/재개/그리드/top_k/소형 end-to-end (torch 필요, ~수십초)
